...
# ----------------- Paths -----------------
csv_path = '/media/love/Love Extern/Output_IdentifySecondaryObjects.csv'
masks_dir = '/media/love/Love Extern/OUTPUT'
output_csv = '/media/love/Love Extern/Output_Cells_NotInsideMasks.csv'
...
def visualize_mask_and_cells(mask, all_cells, kept_cells, image_id, out_path):
    ...
    # All cells
    ax.scatter(all_cells['Location_Center_X'], all_cells['Location_Center_Y'],
               s=10, c='yellow', label='All Cells', alpha=0.3)

    # Kept cancer (CK positive) cells
    cancer_cells = kept_cells[kept_cells['Classify_CK'] == 1]
    ax.scatter(cancer_cells['Location_Center_X'], cancer_cells['Location_Center_Y'],
               s=10, c='red', label='Kept CK+ (Cancer)', alpha=0.8)

    # Kept non-cancer (CK negative) cells
    non_cancer_cells = kept_cells[kept_cells['Classify_CK'] == 0]
    ax.scatter(non_cancer_cells['Location_Center_X'], non_cancer_cells['Location_Center_Y'],
...
for image_id, group in tqdm(df.groupby('ImageID')):
    ...
    for _, row in group.iterrows():
        x = int(round(row['Location_Center_X']))
        y = int(round(row['Location_Center_Y']))
